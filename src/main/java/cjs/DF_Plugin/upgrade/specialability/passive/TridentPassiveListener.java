...
public class TridentPassiveListener implements Listener {
    ...
    public void onDamageByTrident(EntityDamageByEntityEvent event) {
        ...
        // '역류' 능력의 일부로 발사된 투사체는 이 리스너에서 처리하지 않습니다.
        // SpecialAbilityListener가 더 높은 우선순위로 처리하고 이벤트를 취소할 것입니다.
        if (trident.hasMetadata("df_backflow_projectile")) {
            return;
        }
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                for (int i = 0; i < level; i++) {
                    ...
                    // '역류' 능력과 연동하기 위해, 추가 투사체에 특별한 메타데이터를 추가합니다.
                    additionalTrident.setMetadata("df_backflow_projectile", new FixedMetadataValue(plugin, true));
