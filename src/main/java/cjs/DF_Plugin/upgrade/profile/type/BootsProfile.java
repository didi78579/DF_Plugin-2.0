...
public class BootsProfile implements IUpgradeableProfile {
    ...
    public void applyAttributes(org.bukkit.inventory.ItemStack item, ItemMeta meta, int level, List<String> lore) {
        ...
        // 강화 레벨당 이동속도 5% 증가 (과거 코드 로직)
        // Operation.MULTIPLY_SCALAR_1 은 (1 + X)배 이므로, 0.05는 5% 증가를 의미합니다.
        double speedBonusPerLevel = 0.05;
        double totalValue = speedBonusPerLevel * level;

        if (totalValue > 0) {
            AttributeModifier mod = new AttributeModifier(UUID.randomUUID(), ATTRIBUTE_NAME, totalValue, AttributeModifier.Operation.MULTIPLY_SCALAR_1, EquipmentSlotGroup.FEET);
    ...
    private void applyBaseArmorAttributes(Material material, ItemMeta meta) {
        ...
        if (armor > 0) meta.addAttributeModifier(Attribute.GENERIC_ARMOR, new AttributeModifier(UUID.randomUUID(), "generic.armor", armor, AttributeModifier.Operation.ADD_NUMBER, EquipmentSlotGroup.FEET));
        if (toughness > 0) meta.addAttributeModifier(Attribute.GENERIC_ARMOR_TOUGHNESS, new AttributeModifier(UUID.randomUUID(), "generic.armor_toughness", toughness, AttributeModifier.Operation.ADD_NUMBER, EquipmentSlotGroup.FEET));
        if (knockbackResistance > 0) meta.addAttributeModifier(Attribute.GENERIC_KNOCKBACK_RESISTANCE, new AttributeModifier(UUID.randomUUID(), "generic.knockback_resistance", knockbackResistance, AttributeModifier.Operation.ADD_NUMBER, EquipmentSlotGroup.FEET));
    }
