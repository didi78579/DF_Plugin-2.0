...
public class ClanUIManager {
    ...
    public void openDeletionConfirmation(Player player, String clanName) {
        ...
        TextComponent deleteButton = new TextComponent("§c[삭제 진행]");
        deleteButton.setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/df deleteclan confirm"));
        deleteButton.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new ComponentBuilder("§c삭제를 계속 진행합니다.").create()));
        player.spigot().sendMessage(deleteButton);
        player.sendMessage("§e=============================");
    }
